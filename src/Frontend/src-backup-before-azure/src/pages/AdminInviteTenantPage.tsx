...
const Row = ({ invite, fetchInvites }: { invite: any, fetchInvites: () => void }) => {
  ...
  const handleResend = async () => {
    setResending(true);
    try {
      const response = await fetch(`/api/admin/tenants/invites/${invite.id}/resend`, { 
        method: 'POST',
        credentials: 'include' 
      });
      if (response.ok) {
...
export const AdminInviteTenantPage: React.FC = () => {
  ...
  const fetchInvites = async () => {
    try {
      const response = await fetch('/api/admin/tenants/invites', { credentials: 'include' });
      if (response.ok) {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('/api/admin/tenants/invite', {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
