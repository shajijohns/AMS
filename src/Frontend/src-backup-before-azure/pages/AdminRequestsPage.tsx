...
export const AdminRequestsPage: React.FC = () => {
  ...
  const fetchRequests = () => {
    setLoading(true);
    fetch('http://localhost:5200/api/admin/association-requests', { credentials: 'include' })
      .then(res => {
  ...
  const handleAction = async (id: string, action: 'approve' | 'more-info' | 'reject' | 'disapprove', payload?: any) => {
    try {
      const response = await fetch(`http://localhost:5200/api/admin/association-requests/${id}/${action}`, {
        method: 'POST',
