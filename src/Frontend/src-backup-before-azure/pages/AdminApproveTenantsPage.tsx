...
export const AdminApproveTenantsPage: React.FC = () => {
  ...
  const fetchTenants = async () => {
    try {
      const response = await fetch('http://localhost:5200/api/admin/tenants?status=PendingOnboarding', { credentials: 'include' });
      if (response.ok) {
  ...
  const handleAction = async (id: string, action: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:5200/api/admin/tenants/${id}/${action}`, {
        method: 'POST',
        credentials: 'include'
      });
      if (response.ok) {
